...
#define ETEST_TEST(test_group_name, test_name)\
    ...
    \
    void ETEST_CAT2(test_group_name, test_name)::test_body()

#define ETEST_TEST_F(fixture, test_name)\
    class ETEST_CAT2(fixture, test_name) : public fixture {\
    public:\
        std::string group() const override {\
            return std::string(ETEST_TOSTRING(fixture));\
        }\
        std::string name() const override {\
            return std::string(ETEST_TOSTRING(test_name));\
        }\
    ...
    \
    const etest::internal::test_info *ETEST_CAT3(fixture, test_name, holder) =\
        etest::internal::test_suite::instance()->add_test_info(\
            new etest::internal::test_info(\
                ETEST_TOSTRING(fixture),\
                ETEST_TOSTRING(test_name),\
                new etest::internal::test_factory_impl<ETEST_CAT2(fixture, test_name)>));\
    \
    void ETEST_CAT2(fixture, test_name)::test_body()
